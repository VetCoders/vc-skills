...
launch_cmd="set -o pipefail && cd $qroot && { claude -p --output-format stream-json --verbose --dangerously-skip-permissions $model_flag -- \"\$(cat $qruntime)\" 2>&1 | grep --line-buffered '^[[:space:]]*{' | jq --unbuffered -rj -f $qfilter | tee -a $qtranscript; pipeline_status=\$?; echo; { grep -oE 'session: [[:alnum:]][[:alnum:]-]*-[[:alnum:]][[:alnum:]-]*' $qtranscript 2>/dev/null | tail -1 | awk '{print \$2}' | xargs -I{} printf '\\n\\033[33m━━━ session: {} ━━━\\033[0m\\n'; } || true; exit \$pipeline_status; }"
